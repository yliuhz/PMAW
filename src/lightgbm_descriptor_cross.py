...
for i in range(10):
    ...
    for train_index, test_index in kf.split(x_trans1):
        ...
        #                                     n_estimators=1000,
        #                                     verbose=True)
        model_mlp = lgbm.LGBMRegressor(learning_rate=0.05,
                                            max_depth=31,
                                            n_estimators=1000,
                                            verbosity=3)
...
cross_result.to_csv('Out/cross_result_lightgbm.csv', index=False, encoding='gb18030')
...
plt.savefig('pics/descriptor-fig-lightgbm.png')
plt.show()

cross_result = {'Real lambda': in_y_train_real, 'Predicted lambda': in_y_train_pred}
cross_result = pd.DataFrame(cross_result)
cross_result.to_csv('Out/cross_result_lightgbm_train.csv', index=False, encoding='gb18030')