...
def read_bit(filepath):
    ...
    data = np.array(data)
    data_x_df = pd.DataFrame(data[:, :-1])
    data_y_df = pd.DataFrame(data[:, -1])
...
cross_result.to_csv('Out/cross_result_chemfluo.csv', index=False, encoding='gb18030')
...
plt.savefig('pics/descriptor-fig-chemfluo.png')
plt.show()

cross_result = {'Real lambda': in_y_train_real, 'Predicted lambda': in_y_train_pred}
cross_result = pd.DataFrame(cross_result)
cross_result.to_csv('Out/cross_result_chemfluo_train.csv', index=False, encoding='gb18030')